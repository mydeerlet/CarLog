...
import com.mydeerlet.carlog.utils.AppGlobals;
...
public abstract class AppDatabases extends RoomDatabase {
    private static final AppDatabases database;

    static {
        //创建一个内存数据库
        //但是这种数据库的数据只存在于内存中，也就是进程被杀之后，数据随之丢失
        //Room.inMemoryDatabaseBuilder()
        database = Room.databaseBuilder(AppGlobals.getApplication(), AppDatabases.class, "carlog")
                //是否允许在主线程进行查询
    ...
    public static AppDatabases get() {
        return database;
    }
